window.addEventListener('load',()=>{
    ...
    function funNext() {
        if(flag){
            ...
            if(index + 1 === ul.children.length - 1) {
                ...
                ul.style.transform = `translateX(${- container.clientWidth * (ul.children.length - 1)}px)`
                index = 0;
                ...
                // 播放完后跳转到第一张
                // 可用回调函数保证在 transition 结束后再跳转,但可能因js阻塞而失效,故推荐使用 transitionend 事件侦听
                // setTimeout(() => {
                //     ul.style.transition = 'none';
                //     // ul.style.left = '0px';
    ...
    function funPrev() {
        if(flag){
            ...
            if(index - 1 < 0) {
                ...
                ul.style.transform = `translateX(${- container.clientWidth * (ul.children.length - 1)}px)`;
                // 利用回调函数机制，保证 translateX 跳转完毕后再加播放
                setTimeout(() => {
    ...
    function currentSlide(i) {
            ...
            ul.style.transform = `translateX(${- i * container.clientWidth}px)`;
